...
class Network extends BaseBlockWorld implements IBlockWorld {
  ...
  void draw() {
    ...
    for (IActor an : itBlocks()) {
      ...
      for (Arc a : n.nexts) {
        stroke(255,200,100);
        Node dest = (Node)(nthBlock(a.destination));
  ...
  Node getNode(int i) { return (Node)nthBlock(i); }
...
class KDiag extends BaseControlAutomaton implements IAutomatonToy, ICamouseUser, IHasNetwork, IBlockWorld {    
  ...
  void draw() {    
    drawVideo();
    network.draw();
    Node cNode = (Node)(network.nthBlock(currentNode));
    pushMatrix();
      translate(320,240);
  ...
  void mousePressed(int mouseX, int mouseY) { network.mousePressed(mouseX, mouseY); }
  void mouseReleased() { network.mouseReleased(); }  
  void mouseDragged(int mouseX, int mouseY) { network.mouseDragged(mouseX, mouseY); }
